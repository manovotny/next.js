...
mod analyzer_state {
    ...
    impl<'a> Analyzer<'a, '_> {
        ...
        pub(super) fn record_cjs_export(&mut self, name: RcStr, path: AstPath) {
            if let Some(c) = &mut self.state.cjs_exports {
                c.writes.push(CjsExportDrop { name, path });
            }
        }

        /// Returns the removable writes and whether the `__esModule` flag is set.
        pub(super) fn droppable_cjs_exports(&mut self) -> Option<(Vec<CjsExportDrop>, bool)> {
...
impl VisitAstPath for Analyzer<'_, '_> {
    ...
    fn visit_this_expr<'ast: 'r, 'r>(
        ...
        if !self.is_this_bound() {
            ...
            // `this` at the top level of an exported function value is `exports` when the
